<div class="flex">
    ...
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        <div class="flex items-center mr-4  {{ $tweet->isLikeBy( auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
            <svg viewBox="0 0 20 20" class=" mr-1 w-3">
                ...
                <button type="submit" class="text-xs text-gray-600">{{ $tweet->likes ?: 0 }}</button>
        </div>
    </form>
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        ...
        <div class="flex items-center {{ $tweet->isDislikeBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
            <svg viewBox="0 0 20 20" class=" mr-1 w-3" >
                ...
                <button type="submit" class="text-xs text-gray-600">{{ $tweet->dislikes ?: 0 }}</button>
        </div>
